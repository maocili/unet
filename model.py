...
class UNet(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, first_out_channcels: int = 64, steps: int = 4):
        ...
        self.input_layer = DoubleConv(in_channels=in_channels, out_channels=first_out_channcels)
        self.downlayers = nn.ModuleList()
        self.uplayers = nn.ModuleList()
        self.final_layer = nn.Conv2d(first_out_channcels, out_channels, kernel_size=1)

        for i in range(0, steps):
            in_ch = first_out_channcels * 2**i
            out_ch = first_out_channcels * 2**(i+1)
            self.downlayers.append(ContractingBlock(in_channels=in_ch, out_channels=out_ch))

        for i in range(steps, 0, - 1):
            in_ch = first_out_channcels * 2 ** i
            out_ch = first_out_channcels * 2 ** (i - 1)
            self.uplayers.append(ExpansiveBlock(in_channels=in_ch, out_channels=out_ch))
